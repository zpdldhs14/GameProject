    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
				...
				_HandDrawnSpeed = (floor((_Time.x + seed) * 20 * _HandDrawnSpeed) / _HandDrawnSpeed) * _HandDrawnSpeed;
				...
				#if SHAKEUV_ON
				half xShake = sin((_Time.x + seed) * _ShakeUvSpeed * 50) * _ShakeUvX;
				half yShake = cos((_Time.x + seed) * _ShakeUvSpeed * 50) * _ShakeUvY;
            	...
            	#endif
				distortUvs.x += ((_Time.x + seed) * _DistortTexXSpeed) % 1;
				distortUvs.y += ((_Time.x + seed) * _DistortTexYSpeed) % 1;
				#if ATLAS_ON
